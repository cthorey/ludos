...
from ludos.models.playground.gan import data, model
...
def train(config_name, max_epochs=1, maintainer='clement', gpus=1):
    ...
    m = model.Model()
    m.build(cfg.to_dict())
    task = experiment.LightningExperiment(
    ...
    tr.fit(m.network, dm)
    task.upload_checkpoints(
        checkpoint.best_model_path, expname, cfg=m.network.cfg.to_dict())
