...
VOLUMES = [
    ...
    '~/workdir/competitions/hubmap:/workdir:rw',
    '~/.trains.conf:/root/trains.conf:rw', '/mnt/hdd/data:/workdir/data:rw',
...
def create_dataset(data_name,
    ...
    import docker_utils
    cmd = docker_utils.parse_cmd(script="/workdir/hubmap/data/create.py",
                                 fn="create_dataset",
                                 data_name=data_name,
                                 data_type=data_type,
                                 overfit=overfit,
    ...
    docker_client.containers.run(image='hubmap:{}'.format(image_tag),
                                 name=name,
                                 runtime='nvidia',
                                 command=cmd,
                                 detach=True,
...
def train(config_name,
    ...
    docker_client.containers.run(image='xihelm/hubmap:{}'.format(image_tag),
                                 name=name,
                                 runtime='nvidia',
                                 command=cmd,
                                 detach=detach,
